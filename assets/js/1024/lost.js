			...
			for (y=0; y<7; y++) {
				for (x=0; x<5; x++) {
					...
					if (font[text[i]][index]!="0") {
						ctx.fillRect(pX+x*2*scale, pY+y*2*scale + decalY, 3*scale, 3*scale);
					}
					index++;
				}
			}
...
move = function(x, y, p) {
	...
	updateSteps();
	if (can) {
		playSound([0,,0.047,0.5711,0.1329,0.8391,,,,,,,,,,,,,1,,,,,0.3]);
	}
	return can;
}
...
function updateSteps() {
	$("#stepContainer").text(nbMoves);
}

function updateAttempts() {
	attempts--;
	attempts = clamp(attempts, 0, startAttempts);
	$("#attemptContainer").text(attempts);
}

function updateScore() {
	score = score + (10*levelEnCours);
...
function drawScene() {
	...
	for (var i=0; i<players.length; i++) {
		...
		if (p.z > hauteurSol) {
			...
			if (hauteurSol==-1000 && !gameHasEnded) {
				gameOver();
			}
			if (hauteurSol>p.z) {
...
function drawHUDEnd() {
	...
	var aspectR = canvasHUD.width/canvasHUD.height;
	var scale = (canvasHUD.height/80 * aspectR/1.5);

	ctx.fillStyle = '#7F40D9';
	ctx.fillRect(0, 0,  canvasHUD.width, canvasHUD.height);
	t = "1024 MOVES"; drawText(centerX-(scale*6)*(t.length/2), (centerY*.5)-(scale*6), t, 15);
	t = "     CONGRATULATIONS !&YOU MADE IT IN "+nbMoves+" MOVES !&  (BEST POSSIBLE IS 1024)"; 
	drawText(centerX-(scale*6)*(t.length/2), ((centerY*1.6)-(scale*6))+Math.sin(frame/64)*10, t, 7);
...
function drawHUDHome() {
	...
	var aspectR = canvasHUD.width/canvasHUD.height;
	var scale = (canvasHUD.height/80 * aspectR/1.5);
	t = "1024 MOVES"; drawText(centerX-(scale*6)*(t.length/2), (centerY*.5)-(scale*6), t, scale);
	scale = (canvasHUD.height/140);
	t = "PRESS A KEY TO START"; drawText(centerX-(scale*6)*(t.length/2), ((centerY*1.5)-(scale*6))+Math.sin(frame/64)*10, t, scale);
	rafHUD = requestAnimationFrame(drawHUDHome);
}
function drawHUD() {
	ctx.clearRect(0,0,window.innerWidth,window.innerHeight);
	var aspectR = canvasHUD.width/canvasHUD.height;
	var scale = (canvasHUD.height/180 * aspectR);
	if (tutoText) {
		scale = (canvasHUD.height/360 * aspectR);
		drawText(centerX-((scale*6)*(tutoText.length/2)), ((centerY*1.5)-(scale*6)), tutoText.toUpperCase(), scale);
	} else {
		scale = (canvasHUD.height/315) * aspectR;
		drawText(10, 10, nbMoves+" MOVE"+(nbMoves>1 ? "S" : "" ),scale);
		drawText(10, (7*scale*aspectR), attempts + " ATTEMPT"+(attempts>1 ? "S" : "" ),scale);
		scale = (canvasHUD.height/210 * aspectR);
		var t = "STAGE "+levelEnCours;
		scale = (canvasHUD.height/315 * aspectR);
		drawText(centerX-((scale*6)*(t.length/2)), ((centerY*.12)-(scale*6)), t, scale);
		drawText(10, 100, "M - MUTE&R - RETRY", scale);
	}
}
...
switchTileHeight = function(x, y, levelOff, levelOn, state) {
	...
	if (state) {
		if (level[x][y].h==levelOff) {
			...
			playSound([2,0.1,0.20875772806897386,0.4,0.3995907182891785,0.6,0.20781957085118677,-0.15,0,0,0,0,0,0.8997235718602934,-0.19289283657523948,0,0,0,1,0,0,0.0583846933342502,0,0.45]);
		}
	} else {
		if (level[x][y].h==levelOn) {
			level[x][y].inMoveZ = vitesseRotation * (levelOff>levelOn ? -2 : 2);
			playSound([2,0.1,0.20875772806897386,0.4,0.3995907182891785,0.6,0.20781957085118677,0.15,0,0,0,0,0,0.8997235718602934,-0.19289283657523948,0,0,0,1,0,0,0.0583846933342502,0,0.5]);
		}
	}
...
function resize() {
	...
	canvas.width = clamp($("#game-container").width(), 600, 1500);
	canvas.height = clamp($("#game-container").width()*.5, 300, 750);
	$("#game-container").height(canvas.height);
...
function _1024() {
	startAttempts = $("input[name='attempts']").val();
