$(document).ready( function() {
    ...
    addNewTinyMCE("add_blog", "", 8000, aWidth, aHeight);	
    ...
    function addBlogPost(id, t, su, p, e, fi, c) {
        $.ajax({
            method: 'POST',
            data: ({ id: id, title: t, shorturl: su, published: p, excerpt: e, featured_image: fi, content: c}),
            url: baseURL + "/blog/addBlogPost",
            error: function() {
                showSweetAlert("Could not add blog post.", "Whoops!", "error");
            },
        ...
        return $.ajax({
            ...
            success: function(data) {
                ...
                if (data.status == "success") {
                    ...
                    $("#addBlogPost").attr("disabled", false);
                } else {
                    $("#shorten_url").css("background-color", "rgb(187 0 0 / 37%)");
                    $("#addBlogPost").attr("disabled", true);
                }
            }
        });
    }
    ...
    $("#addBlogPost").click( function(e) {
        ...
        var content = tinymce.get('add_blog').getContent();

        if (tinymce.get('add_excerpt').isDirty() || tinymce.get('add_blog').isDirty()) {
            showSweetConfirm("Create new blog post?", "Attention", $icon='info', function(confirmed) {
                if (!confirmed) {
                    e.preventDefault();
                } else {
                    addBlogPost(id, title, shorturl, publishedState, excerpt, image, content);
                }
            });
        } else {
