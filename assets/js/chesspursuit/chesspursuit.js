...
aa.add( 'check', 1,
	[
		[2,,0.1747,,0.1291,0.6731,0.2,-0.2999,,,,,,0.4368,0.1862,,0.28,,1,,,0.1596,,0.25]
	]
);
aa.add( 'move', 5,
	[
		[3,,0.0316,,0.1483,0.5871,,-0.6609,,,,,,,,,,,1,,,0.0227,,0.15]
	]
);
aa.add( 'capture', 5,
	[
		[0,,0.3065,,0.2516,0.36,,0.1584,,,,,,0.1149,,,,,0.6,,,0.2188,,0.25]
	]
);
aa.add( 'checkmate', 1,
	[
		[1,,0.2402,,0.3917,0.2242,,0.1535,,,,,,,,0.5609,,,1,,,,,0.2]
	]
);

})();;

function ChessPursuit(){
	"use strict";
	...
	// let highestScoreContainer    = document.querySelector(".highestScore-container");
	// let fastestTimeContainer    = document.querySelector(".fastestTime-container");
	...
	var startAttempts = $("input[name='attempts']").val();
	...
	function init(){
		...
		offsetLeft = getOffsetLeft(root);
		offsetTop = getOffsetTop(root);
	...
	function updateTime() {
		...
		time = (Date.now() - roundStartTime) * MS_TO_S;
		
		// timeContainer.textContent = time.toString().toHHMMSS();
	}
	...
	function updateHighestScore() {
		if (score > bestScore || (score == bestScore && time < bestTime)) {
			bestScore = score;
			bestTime = time;
			// highestScoreContainer.textContent = bestScore + " - " + time.toString().toHHMMSS();
			// fastestTimeContainer.textContent = bestScore + " - " + time.toString().toHHMMSS();
		}
	}
	
	function updateFastestTime() {
		if ((time < bestTime && score >= bestScore) || bestTime == 0) {
			bestTime = time;
			// fastestTimeContainer.textContent = score + " - " + bestTime.toString().toHHMMSS();
		}
	}
	...
	function initCheckBoard(startIndex){
		if(checkBoard){
			destroyCheckBoard();
		}
		checkBoard = [];
	...
	function tic(){
		if ((attempts == 0 && gameHasEnded) || startAttempts == 0) {
		...
		if(gameIsOver && !gameHasEnded){
			if(keys.space === 0 || mouse.click){
				setGameIsOver(false, false);
				keys.space = -1;
    ...
    function processInput(){
        ...
        if(player.invalid || intro || gameHasEnded){
	...
	function update(){
		if (roundStartTime == 0 && introStep == 9) {
	...
	function initSvg(){
		...
		function makeGameOverScreen(){
            ...
            svgInnerHtml(text,
                ...
                '<tspan x="50%" dy="1.5em">to restart.</tspan>'
            );
            gameOverScreen.appendChild(text);
		}
		
		function makeEndScreen(){
			gameEndScreen = document.createElementNS(xmlns, "g");
			gameEndScreen.style.display = 'none';
			svgElem.appendChild(gameEndScreen);

			var rect = document.createElementNS(xmlns,'rect');
			svgAttrs(rect, {x:0, y:0, width:'100%',height:'100%',fill:'rgba(0,0,0,0.5)'});
			gameEndScreen.appendChild(rect);
			...
			svgInnerHtml(text, 'GAME OVER !');
			gameEndScreen.appendChild(text);
		}
	...
	document.onmousemove = function(e){
		mouse.x = e.clientX - (offsetLeft - SIZE/2);
